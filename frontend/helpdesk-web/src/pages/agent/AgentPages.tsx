  ...
  return (
    <div>
      ...
      <div className="d-flex gap-2 mb-3 flex-wrap">
        ...
        {(search || status) && <Button variant="outline-secondary" size="sm" onClick={() => { setSearch(''); setStatus(''); }}>Clear Filters</Button>}
      </div>
      {items.length === 0 ? <EmptyState title="No unassigned tickets" desc="Your queue is clear." action={<Button variant="outline-secondary" onClick={() => { setSearch(''); setStatus(''); }}>View Resolved Tickets</Button>} />
        : <><Table striped hover responsive className="desktop-table"><thead><tr><th>Ticket</th><th>Subject</th><th>Priority</th><th>Status</th><th>SLA</th><th>Updated</th></tr></thead><tbody>{items.map((t) => (<tr key={t.id}><td><Link to={`/agent/tickets/${t.id}`}>{t.ticketNumber}</Link></td><td>{t.subject}</td><td><Badge bg="warning">{t.priority}</Badge></td><td><Badge bg="info">{t.status}</Badge></td><td>{t.slaStatus}</td><td>{new Date(t.createdAt).toLocaleString()}</td></tr>))}</tbody></Table>
          <div className="mobile-cards">{items.map((t) => (<Link key={t.id} to={`/agent/tickets/${t.id}`} style={{ textDecoration: 'none', color: 'inherit' }}><Card className="mb-2 p-3 ticket-card"><strong>{t.ticketNumber}</strong><div className="fw-semibold">{t.subject}</div><div className="d-flex gap-2 mt-1"><Badge bg="warning">{t.priority}</Badge><Badge bg="info">{t.status}</Badge></div><small className="text-secondary">Updated {new Date(t.createdAt).toLocaleString()}</small></Card></Link>))}</div>
          <p className="text-secondary">Showing 1–{items.length} of {data?.total ?? 0} tickets</p></>}
    </div>
  );
}
